...
> 注：ADR-12~15（M2 评估 / M3 检索入库）记录在 `plan-m2--20260705--v1.md` §-ADR 与 `plan-m3--20260705--v1.md` §7，尚未回并至本文，属 M2/M3 收尾项。

**ADR-16 Agent 范式分层选型（M4）** → 选 **三层范式**：总控层 = Router/Intent-routing；RAG 质量控制层 = Reflection / Corrective RAG（rerank 分数客观信号触发，非 LLM 自评）；工具编排层（GIS）= 预留 ReAct。备选：全局纯 ReAct、全局纯 Plan-and-Execute。理由：科研意图有限且清晰，Router 比 ReAct 更可控/省 token/可测；质量控制用客观信号避免自我偏好偏差（呼应 ADR-06/§5.2）；开放式工具编排才用 ReAct，限定节点内不污染总控。Trade-off：不如纯 ReAct 自主，但可控/可测/省钱，符合 ADR-01。详见 `plan-m4--20260705--v1.md` §3。

**ADR-17 编排骨架完整 + 能力按里程碑填（M4）** → M4 实现完整 5 分支意图路由骨架，只真接 SEARCH+ASK（M1/M3 能力），PLOT/GIS/WRITE 为占位节点。备选：只画图不接 / 5 条全实现。理由：骨架完整体现设计能力，占位体现编排/能力解耦（填能力不动编排层）+ 诚实边界。Trade-off：3 个分支是占位，但为可讲加分项。

**ADR-18 CRAG 检索分数门控自主补充（M4）** → RAG_QA 是否触发联网补充由 `top1 rerank 分 < 阈值`（config）决定，非 LLM 自评；有 `crag_max_rounds` 上限。备选：LLM 判"够不够答"。理由：客观信号避免自我偏好偏差、可量化可复现（呼应 §5.2 三坑）。Trade-off：阈值需调参，但比 LLM 拍脑袋可控。

**ADR-19 人机中断 interrupt + SqliteSaver 持久化（M4）** → HUMAN_REVIEW 用标准 `interrupt()` + SqliteSaver，支持单命令内 resume（流畅）与跨进程 `--resume`（持久）。备选：节点内 `input()`（无 LangGraph 特性）、仅 MemorySaver（不能跨进程）。理由：既流畅演示又能讲断点续跑/状态持久化。Trade-off：多一个 checkpoint-sqlite 依赖 + 测试量最大。
